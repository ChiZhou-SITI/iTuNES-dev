...
for i in range(len(trans_name)):
	if transcript_seq[i]=="NULL":
		...
		wt_head='>WT_'+gene_symbol[i]+'_'+ ref_animo_acid[i]+str(pro_change_pos)+ alt_animo_acid[i]
		mt_head='>MT_'+gene_symbol[i]+'_'+ ref_animo_acid[i]+str(pro_change_pos)+ alt_animo_acid[i]
...
snv_fasta_dd=snv_fasta_out.drop_duplicates(subset=['mutation_header','mutation_peptide','wild_header','wild_peptide'])
data_snv_dd_reindex=snv_fasta_dd.reset_index()
