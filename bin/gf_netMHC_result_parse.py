...
opts,args=getopt.getopt(sys.argv[1:],"hi:t:g:o:b:f:s:",["input_netmhc_file","input_genefusion_fasta","ericscript_outfile","out_dir","binding_affinity_cutoff","exp_cutoff","sample_id"])
input_netmhc_file=""
...
if (input_netmhc_file =="" or input_genefusion_fasta=="" or ericscript_outfile =="" or out_dir =="" or sample_id==""):
	print USAGE
	sys.exit(2)		
...
for i in range(len(MT_neo)):
	...
	line=re.split(r'\s+',MT_neo[i][j].strip())
        if line[-1]=="WB" or line[-1]=="SB":
            ...
            fusion_gene.append(MT_header[i])
...
data_select_filter_sorted=data_select_filter.sort(["Binding_Aff"],ascending=True)
